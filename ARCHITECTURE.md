...
| DNS | FakeIP выключен; Android bootstrap + managed DNS; DoH fallback `parallel` |
...
14. Управляемый DNS запускает три равноценных DoH параллельно только при cache miss; первый корректный ответ выигрывает, остальные запросы отменяются.
...
13. Проверить соединение с сервером, DNS через VPN и основной HTTPS endpoint; резервный endpoint вызвать один раз только после реальной ошибки основного.
...
Включённая по умолчанию настройка «Только IPv4 через VPN» влияет только на DNS-ответы доменов, которые effective route отправит в proxy. Она действует в Auto, Secure и DNS Android, но не меняет direct/LAN, IPv6-маршрут TUN, сохранённый профиль или режим «Из JSON». Это предотвращает выбор AAAA у dual-stack сайтов для WireGuard-профиля только с внутренним IPv4. IPv6-only трафик требует настоящего IPv6-адреса внутри WireGuard-профиля; отключение фильтра не может добавить его автоматически.
...
| Три DoH параллельно | Используем для managed fallback: exact core с `sequential` не достигает резерва, если первый DoH занял общий deadline; запросы возникают только на cache miss |
...
- DNS cache остаётся включённым с capacity 4096. Managed fallback использует `parallel`: Cloudflare, Google и OpenDNS стартуют только на cache miss, потому что `sequential` exact core не достигает резерва при зависании первого;
