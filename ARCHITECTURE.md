...
> **Читать первым.** Это главный документ для человека или ИИ, продолжающего проект. Он фиксирует границы MVP, владельцев данных, порядок реализации и запрещённые усложнения. Детали сетевой политики находятся в [ROUTING_ARCHITECTURE.md](ROUTING_ARCHITECTURE.md), детали DNS и Android VPN — в [DNS_ARCHITECTURE.md](DNS_ARCHITECTURE.md), границы входных форматов — в [IMPORT_FORMATS.md](IMPORT_FORMATS.md), рабочие TODO и gates — в [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md).
...
| Текущий workspace | Один Android-модуль, profile/JSON/routing/session UI, один VPN service, pinned libbox и packaged `.srs` |
...
4. Политика форматов определяет допустимые преобразования URI/subscription в JSON.
5. Поведение ядра определяется только зафиксированным исходным commit.
...
- по умолчанию пропускает через VPN только выбранные приложения, экономя VPN-сервер;
...
| Архитектура проекта | Один Gradle-модуль `app` |
...
| DNS | FakeIP выключен; Android bootstrap + managed DNS; DoH fallback `sequential` |
...
14. Управляемый DNS не дублирует успешный запрос двум DoH: основной и резервный вызываются последовательно.
...
7. Добавить только разрешённые overlays: health-check и при необходимости bootstrap LKG.
...
13. Проверить соединение с сервером, DNS через VPN и один из двух HTTPS endpoints.
...
`CommandLog` до экрана диагностики не создаётся.

Пинг на карточке — время одного HTTPS round-trip через фактически выбранный
маршрут, а не ICMP. Первый результат переиспользует обязательный connect
health-check; дальше проверка выполняется только кнопкой пользователя. Внешний IP
...
До create/update/restore вызывается native `Libbox.checkConfig()`. Файл и буфер ограничены 4 МБ; файл читается только после ответа системного `OpenDocument`, буфер — только в обработчике явного нажатия и на main thread для совместимости с API 26. Credentials никогда не копируются в index или DataStore.

Глобальный include allowlist хранится отдельно в DataStore `vpn_scope`. Флаг `initialized` отличает первый запуск от осознанно очищенного списка: установленные popular suggestions выбираются только один раз и больше не возвращаются после ручного снятия. Каталог пакетов читается лениво при открытии picker, а не фоново и не по таймеру. Собственный package приложения отбрасывается при чтении/записи DataStore и скрыт из каталога. Полный список нужен основной per-app функции, обрабатывается только локально и никогда не попадает в сеть, аналитику или диагностику.

Перед будущим `Builder.establish()` единственный `VpnAppScopePreflight` нормализует пользовательский список, отклоняет пустой список и отсутствующие/отключённые пакеты, затем добавляет пользовательские package и внутренний package Zapret KVN через `addAllowedApplication()`. Любое исключение Builder возвращает блокирующий результат; частично заполненный Builder после ошибки не используется. Внутренний package существует только в effective platform allowlist для health-check и никогда не сохраняется как пользовательский выбор.
...
Один профиль — один настоящий sing-box JSON. Серверы находятся в его массиве `outbounds`; отдельной таблицы серверов и связанного набора полноразмерных JSON-шаблонов нет.
...
Raw JSON не нормализуется скрыто. В режиме «Из JSON» GUI показывает существующие selector-группы как есть. Для включения managed DNS/routing пользователь явно выбирает существующий основной selector либо разрешает создать `zapret-proxy`.
...
- исчезнувший package или Builder-ошибка отменяет запуск целиком;
...
В APK хранится только небольшой список подсказок: Instagram, YouTube, Telegram, WhatsApp, Discord, Signal и популярные браузеры; TikTok исключён. Неизвестные приложения автоматически не добавляются. Системные скрыты до нажатия «Показать системные».
...
- **Автоматически** — Android DNS для bootstrap/LAN/direct, DoH через proxy для proxy-доменов;
- **DNS Android** — системная DNS/Private DNS политика;
- **Защищённый через VPN** — стандартный DNS выбранных приложений через DoH/proxy;
- **Из JSON** — профиль не переписывается.

FakeIP выключен. Системные настройки Android приложение не меняет и Private DNS не обходит.

Managed Auto/Secure не запускаются при strict Private DNS, потому что системный DoT может обойти port-53 hijack и reverse mapping. Полная политика, bootstrap cache и fail-close описаны в DNS ADR.
...
| Два DoH параллельно | Отклонено для managed default: два запроса и две активные HTTPS-сессии на cache miss |
| Отдельный VPN-процесс | Отклонён до профилирования: добавляет IPC/process overhead и усложняет lifecycle |

Pinned Android AAR собирается с `with_gvisor`. Для managed TUN поле `stack` не переопределяем: exact core выбирает upstream default `mixed`; пустой `mtu` получает Android-specific default ядра `9000`. Отдельные переключатели stack/MTU в обычном GUI не добавляем до измерений на API 26 и современной ОС. Пользовательский JSON сохраняет право задать их явно.
...
- `CommandStatus` с интервалом 1 секунда включается только для видимой главной; `CommandConnections` не включается никогда, `CommandLog` — только на открытом экране диагностики;
...
- DNS cache остаётся включённым с capacity 4096. Managed fallback использует `sequential`: второй DoH вызывается только после transport-ошибки первого; `parallel` доступен только в явном пользовательском JSON;
...
7. Серия уникальных DNS-имён — `sequential` против контрольного `parallel` по числу запросов, CPU и энергии.
...
└─ src/main/java/.../
   ...
   ├─ vpn/          VpnService, VpnController, lifecycle/platform adapter
   ├─ profiles/     ProfileStore и metadata index
   ├─ config/       JSON tree editor, validation, presets, rule-set assets
   ├─ importer/     QR/clipboard/URL/file/subscription parsing
   ├─ updates/      GitHub Release check/download/install handoff
   └─ diagnostics/  redaction, logs, export
```

Не создаём `domain/`, `usecase/`, repository interfaces для каждого файла или отдельные Gradle-модули. Конкретные небольшие классы передаются через один `AppContainer`. `VpnService` остаётся единственным владельцем активного PFD/core lifecycle.
...
Диагностика показывает короткий тип ошибки и раскрываемые последние 80 redacted-строк текущего запуска. Отдельный `CommandLog` существует только пока экран видим и Activity находится в `STARTED`; runtime-лог на диск не пишется. Diagnostic JSON создаётся только явной кнопкой, не содержит raw profile, package list, endpoint, внешний IP или credentials и включает app/core revision, Android/API, non-VPN network/Private DNS и структурную сводку effective `zapret-*` overlay. Временный файл передаётся системным Sharesheet через non-exported `FileProvider` с read grant и удаляется при следующем запуске.

Updater проверяет только GitHub Releases Stable/Beta и только после явной кнопки. Он требует согласованные `release-metadata.json`, отдельный SHA-256 и GitHub digest, ограничивает HTTPS/redirect/размер, затем проверяет package, повышение versionCode, minSdk и signing history содержимого APK. Лишь после этого APK из `cache/updates` передаётся штатному Android installer через bounded non-exported FileProvider. Ошибка, отмена и следующий запуск удаляют временный APK. Core никогда не скачивается отдельно: libbox меняется только вместе с подписанным APK.

Release workflow строит CLI/AAR/APK из одного полного commit, проверяет embedded revision,
подписывает постоянным ключом из GitHub environment и публикует arm64 APK, checksum,
metadata и release notes. Assets существующего tag не заменяются. Порядок создания и
...
- четыре DNS-режима;
- port-53 hijack, DoH fallback/sequential;
...
- подписанный exact-core arm64 APK, metadata, SHA-256 и notes;
...
- 6/6 JSON приняты CLI, собранным из точного commit;
- 6/6 приняты compatibility release CLI;
...
- одномодульный Gradle-проект собирает debug и R8 release APK; release содержит только `arm64-v8a`, один process и один `VpnService`;
...
| P3 | Оптимальный MTU и отсутствие OEM/операторских проблем у default `9000` | Проверить IPv4, IPv6, NAT64, QUIC, крупные загрузки, PMTU/fragmentation и смену Wi‑Fi/mobile; сравнить default с безопасными кандидатами только при воспроизводимом сбое | Не задавать managed `mtu` и не добавлять UI-настройку |
...
| P15 | Надёжность экономичного DNS fallback | Primary success/error/hang, backup success/error, RCODE и cache burst; сравнить `sequential` с `parallel` по энергии и запросам | Managed default `sequential`; при полном timeout — fail-close, без скрытого перехода на plaintext DNS |
