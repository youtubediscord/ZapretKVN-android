...
| P16 | Устраняет ли Android WireGuard roaming patch зависание после handshake на проблемном устройстве | Test 13: тот же профиль, Wi-Fi и DNS-режим; сравнить handshake → DNS/HTTPS с Test 12, затем повторить mobile/network switch | Patch включён как официальный Android-compatible contract; не объявлять первопричину окончательно доказанной до device A/B |
