...
| Два DoH параллельно | Используем для managed fallback: exact core с `sequential` не достигает второго DoH, если первый занял общий deadline; запросы возникают только на cache miss |
...
- DNS cache остаётся включённым с capacity 4096. Managed fallback использует `parallel`: оба доверенных DoH стартуют только на cache miss, потому что `sequential` exact core не достигает резерва при зависании первого;
