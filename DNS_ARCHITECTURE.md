...
Во всех четырёх режимах runtime-копия ставит узкое правило точных HTTPS probe-hostnames → selected outbound до пользовательских destination rules. Поэтому проверка не может успешно уйти через `direct` IPv6 у WireGuard-профиля только с `0.0.0.0/0`. В managed-режимах и при минимальном Android fallback перед ним также стоит `port: 53 → hijack-dns`. `package_name` здесь не используется: Android Builder уже допустил только Zapret KVN и пользовательскую allowlist, поэтому повторная package-проверка не нужна. Probe-hostnames принадлежат только health-check, не сохраняются в профиль и не используются приложением для других операций.
