...
| `dns/transport/fallback/strategy.go` | `sequential` переходит дальше только при transport/Go-ошибке и использует тот же context; зависший первый сервер может исчерпать весь deadline. `parallel` запускает оба транспорта в общем bounded context | managed default — `parallel`: это единственный штатный fallback exact core, который действительно переживает зависание первого DoH; plaintext fallback запрещён |
...
Настройка «Только IPv4 через VPN» добавляет `strategy: "ipv4_only"` к сгенерированным DNS-действиям для доменов, которые по effective route пойдут через proxy. В Auto/Secure это правила `zapret-secure-dns`; в «DNS Android» те же доменные правила остаются на системном resolver, но получают `ipv4_only`. Pinned sing-box отвечает на AAAA пустым успешным ответом, поэтому приложение выбирает A и не тратит время на IPv6, которого нет у proxy-сервера. LAN/direct-домены, IPv6 TUN route и сохранённый JSON не меняются. Настройка включена по умолчанию и не применяется только в «Из JSON».
...
Managed extended `fallback` использует `parallel`. Оба DoH-транспорта стартуют только на cache miss и работают в одном bounded context; первый корректный DNS-пакет выигрывает, остальные вызовы отменяются. Это создаёт до двух защищённых запросов на новый домен, но в exact core является единственным штатным способом не потерять резервный сервер, когда основной завис до общего deadline. Кэш capacity 4096 ограничивает повторение этой цены. Периодического трафика, отдельного health-loop и plaintext fallback нет.

`fallback` не оценивает DNS RCODE: `NXDOMAIN`, `SERVFAIL` или `REFUSED` являются корректным DNS-пакетом и могут выиграть гонку. Поэтому оба managed endpoints должны быть равно доверенными и семантически взаимозаменяемыми. Пользовательский режим «Из JSON» сохраняет явно заданную стратегию без подмены.
...
Один внешний HTTPS URL не является единственным критерием подключения: перед ним обязательны bootstrap/proxy и DNS-пробы. MVP сначала выполняет один небольшой HTTPS probe Cloudflare через выбранный outbound и только после его ошибки один раз пробует Google. Любой корректный TLS+HTTP ответ 2xx–5xx доказывает полезный путь; точный код 204 не требуется. Обе ошибки закрывают VPN. Внешний IP остаётся дополнительной диагностикой, а не блокирующим условием. Эти запросы существуют только при подключении/контролируемом restart: периодического опроса нет.

Если underlying network работает, но proxy или активный DNS не восстановились в пределах bounded health deadline (15 секунд для DNS+HTTPS части):
...
- **Только IPv4 через VPN** — включённая по умолчанию опция для Auto/Secure/Android DNS: подавляет AAAA только для доменов, направляемых через proxy; direct/LAN и пользовательский JSON остаются dual-stack. Для IPv6-only proxy-сайта опцию можно выключить, но WireGuard-профиль при этом обязан иметь собственный внутренний IPv6-адрес.
...
| Эталон | SHA-256 |
|---|---|
| [Android DNS](testdata/dns/android-dns.json) | `33e49eada092ce9dcc5e718734d71f268102fd050be0aeeb78561e3fee5294e4` |
| [Auto, proxy-final](testdata/dns/auto-proxy-final.json) | `5ab1e656bb76b1808199f5ff4fb551e4bb140b6f0375ca6d01e2adc1d9c1daed` |
| [Auto, direct-final](testdata/dns/auto-direct-final.json) | `da781a9b966cfebb44c3b974cf55a13b1ea1e1573c6c072495841dad57e227b6` |
...
- оба DoH стартуют на cache miss: один успешен/быстро недоступен/завис, оба недоступны, первый корректный ответ выигрывает; отдельно проверяются гонки с `NXDOMAIN`/`SERVFAIL`/`REFUSED`;
