...
Необязательная настройка «Только IPv4 через VPN» добавляет `strategy: "ipv4_only"` только в сгенерированные DNS route-действия, направленные на `zapret-secure-dns`. Pinned sing-box отвечает на AAAA пустым успешным ответом, поэтому приложение выбирает A и не тратит время на IPv6, которого нет у proxy-сервера. Правила `zapret-android-dns`, LAN/direct-домены, IPv6 TUN route и сохранённый JSON не меняются. Настройка выключена по умолчанию, не применяется в «DNS Android»/«Из JSON» и потому не ломает обычный IPv6/NAT64 без явного действия пользователя.
...
- **Только IPv4 через VPN** — выключенная по умолчанию опция для Auto/Secure: подавляет AAAA только в managed DoH через proxy; полезна, если сервер не имеет IPv6 egress. Direct/LAN и пользовательский JSON остаются dual-stack.
