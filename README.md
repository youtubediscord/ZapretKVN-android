...
- **Без фонового опроса.** Нет периодических health-check, бесконечного reconnect или фоновой синхронизации. Cloudflare, Google и OpenDNS используются параллельно только на DNS cache miss, потому что exact core не достигает резерва при зависшем первом сервере.
