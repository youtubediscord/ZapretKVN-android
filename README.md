...
> **Stable `0.2.2` опубликован.** Физическая матрица слабых устройств,
...
[Скачать stable 0.2.2](https://github.com/youtubediscord/ZapretKVN-android/releases/tag/v0.2.2) · [Посмотреть архитектуру](ARCHITECTURE.md) · [Изучить маршрутизацию](ROUTING_ARCHITECTURE.md)
...
Для WireGuard внешний TUN сохраняет значение профиля/ядра, а endpoint без собственного
`MTU` получает Android-совместимый внутренний MTU 1280, как в официальном клиенте
...
Нормализация внешнего Android TUN до 1500 применяется к обычным proxy-профилям, но
автоматически не применяется к userspace WireGuard. Для него сохраняется MTU TUN из
профиля или ядра: физический A/B показал, что сочетание внешнего TUN 1500 и внутреннего
WireGuard 1280 пропускает handshake и короткие DNS-пакеты, но может блокировать TCP/TLS.
...
для профилей без userspace WireGuard.
...
key. `0.2.2` обновляет `0.2.1` и `0.2.1-beta.30` без потери app-private данных. Ручные
