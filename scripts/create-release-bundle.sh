...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Для userspace WireGuard внешний Android TUN больше не принудительно получает MTU 1500: сохраняется значение профиля/ядра, чтобы не блокировать TCP/TLS при внутреннем MTU 1280.' \
